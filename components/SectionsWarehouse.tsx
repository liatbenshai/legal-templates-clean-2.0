...
export default function SectionsWarehouse({ onAddSection, selectedWillType }: SectionsWarehouseProps) {
  ...
  const categories = [
    { id: 'all', name: 'כל הקטגוריות', icon: '📋', count: sectionsWarehouse.length },
    { id: 'property', name: 'נכסים', icon: '🏠', count: sectionsWarehouse.filter(s => s.category === 'property').length },
    { id: 'inheritance', name: 'ירושה', icon: '👨‍👩‍👧‍👦', count: sectionsWarehouse.filter(s => s.category === 'inheritance').length },
    { id: 'restrictions', name: 'הגבלות', icon: '🚫', count: sectionsWarehouse.filter(s => s.category === 'restrictions').length },
    { id: 'special', name: 'מיוחד', icon: '⭐', count: sectionsWarehouse.filter(s => s.category === 'special').length },
    { id: 'family', name: 'משפחה', icon: '👪', count: sectionsWarehouse.filter(s => s.category === 'family').length },
    { id: 'business', name: 'עסקים', icon: '🏢', count: sectionsWarehouse.filter(s => s.category === 'business').length }
  ];

  const filteredSections = [...sectionsWarehouse, ...customSections].filter(section => {
  ...
  const handleCreateCustomSection = (improvedText: string) => {
    ...
    const saved = JSON.parse(localStorage.getItem('customWillSections') || '[]');
    saved.push(newSection);
    localStorage.setItem('customWillSections', JSON.stringify(saved));
  };
