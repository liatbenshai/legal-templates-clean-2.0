...
export default function ProfessionalWordExporter({
  ...
  const exportToWord = async () => {
    ...
    try {
      ...
      if (witnessGender === 'both-male') {
      ...
      } else if (witnessGender === 'both-female') {
        ...
        witnessDeclaration = `אנו מעידות/מעידים בזאת שהמצווה הנ"ל ${willData.testator?.fullName || '[שם מלא]'}, הנושא תעודת זהות ${willData.testator?.id || '[ת.ז]'} חתם/ה בנוכחותנו על צוואתו/ה הנ"ל לאחר שהצהיר/ה בפנינו שזאת צוואתו/ה האחרונה שאותה עשה/תה מרצונו/ה הטוב והחופשי בהיותו/ה בדעה צלולה ובלי כל אונס או כפיה, וביקש/ה מאיתנו להיות עדות/עדים לחתימתו/ה ולאשר בחתימת ידנו שכך הצהיר וחתם בפנינו. ועוד אנו מצהירות/י כי אנו לא קטינות/ים ולא פסולות/פסולי דין וכי אין בינינו ובין המצווה יחס של קרבה כלשהיא, אין לנו כל טובת הנאה בעיזבון המצווה הנ"ל, והננו חותמות/ים ומאשרות/ים בזה כי המצווה הנ"ל חתם בפנינו על שטר צוואה זה לאחר שהצהיר בפנינו כי זו צוואתו ובזה אנו חותמות/ים בתור עדות/ים לצוואה בנוכחות של המצווה הנ"ל ובנוכחות כל אחת/אחד מאיתנו.`;
      }
