...
export default function ProfessionalWordExporter({
  ...
  const exportToWord = async () => {
    ...
    try {
      ...
      // יצירת הצהרת עדים עם gender inflection
      const { replaceTextWithGender } = require('@/lib/hebrew-gender');
      
      let witnessDeclaration = `אנו מעידים/ות בזאת שהמצווה/ה הנ"ל ${willData.testator?.fullName || '[שם מלא]'}, הנושא/ת תעודת זהות ${willData.testator?.id || '[ת.ז]'} חתם/ה בנוכחותנו על צוואתו/ה הנ"ל לאחר שהצהיר/ה בפנינו שזאת צוואתו/ה האחרונה שאותה עשה/תה מרצונו/ה הטוב והחופשי בהיותו/ה בדעה צלולה ובלי כל אונס או כפיה, וביקש/ה מאיתנו להיות עדים/ות לחתימתו/ה ולאשר בחתימת ידנו שכך הצהיר/ה וחתם/ה בפנינו. ועוד אנו מצהירים/ות כי אנו לא קטינים/ות ולא פסולי/ות דין וכי אין בינינו ובין המצווה/ה יחס של קרבה כלשהיא, אין לנו כל טובת הנאה בעיזבון המצווה/ה הנ"ל, והננו חותמים/ות ומאשרים/ות בזה כי המצווה/ה הנ"ל חתם/ה בפנינו על שטר צוואה זה לאחר שהצהיר/ה בפנינו כי זו צוואתו/ה ובזה אנו חותמים/ות בתור עדים/ות לצוואה בנוכחות של המצווה/ה הנ"ל ובנוכחות כל אחד/ת מאיתנו.`;

      // החלת gender inflection על הצהרת העדים
      if (bothFemale) {
        // שתי העדות נקבות
        witnessDeclaration = replaceTextWithGender(witnessDeclaration, 'female');
      } else if (bothMale) {
        // שני העדים זכרים  
        witnessDeclaration = replaceTextWithGender(witnessDeclaration, 'male');
      } else {
        // עדים מעורבים - זכר ונקבה (ברירת מחדל)
        witnessDeclaration = replaceTextWithGender(witnessDeclaration, 'male');
      }
