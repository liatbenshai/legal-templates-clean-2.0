    ...
    try {
      ...
      if (willData.type === 'mutual') {
        sections.push(
          new Paragraph({
            children: [
              new TextRun('בהיות אין אדם יודע יום פקודתו.')
            ],
        ...
        sections.push(
          new Paragraph({
            children: [
              new TextRun('הואיל כי אין אדם יודע את יום פקודתו;')
            ],
            spacing: { after: SPACING.betweenParagraphs }
          })
        );
        ...
        sections.push(
          new Paragraph({
            children: [
              new TextRun(`והואיל כי ברצוני לערוך את צוואתי, ולפרט את רצוני האחרון והוראותיי בכל הקשור לאשר ייעשה ברכושי לאחר פטירתי, לאחר אריכות ימים ושנים;`)
            ],
            spacing: { after: SPACING.betweenParagraphs }
          })
        );
        ...
        sections.push(
          new Paragraph({
            children: [
              new TextRun(`והואיל כי הנני למעלה מגיל שמונה עשרה שנים, ${gender ? 'אזרחית ישראלית ותושבת' : 'אזרח ישראלי ותושב'} מדינת ישראל;`)
            ],
            spacing: { after: SPACING.betweenParagraphs * 1.5 }
          })
        );
        ...
        new Paragraph({
          ...
          children: [
            new TextRun(willData.type === 'mutual' 
              ? 'למען הסר ספק, אנו מבטלים בזה ביטול גמור, כל צוואה ו/או הוראה שנתנו בעבר לפני תאריך חתימה על צוואה זו, בין בכתב ובין בעל פה בקשור לרכושנו ולנכסנו.'
              : 'למען הסר ספק, אני מבטל בזה ביטול גמור, מוחלט ושלם, כל צוואה ו/או הוראה שנתתי בעבר לפני תאריך חתימה על צוואה זו, בין בכתב ובין בעל פה בקשור לרכושי ולנכסיי.'
            )
          ]
        })
      );
      ...
      sections.push(
        new Paragraph({
          heading: HeadingLevel.HEADING_1,
          children: [new TextRun('היקף העיזבון')]
        })
      );
      ...
      sections.push(
        new Paragraph({
          heading: HeadingLevel.HEADING_1,
          children: [new TextRun('חלוקת העיזבון')]
        })
      );
      ...
      if (willData.heirs && willData.heirs.length > 0) {
        ...
        sections.push(
          new Table({
            columnWidths: [2680, 2340, 2340, 2000], // מימין לשמאל!
      ...
      sections.push(
        new Paragraph({
          numbering: { reference: 'main-numbering', level: 0 },
          children: [
            new TextRun('במקרה של פטירת אחד מהיורשים הנזכרים לעיל לפני פטירתי, חלקו יעבור ליורשיו החוקיים.')
          ]
        })
      );
      ...
      sections.push(
        new Paragraph({
          numbering: { reference: 'main-numbering', level: 0 },
          children: [
            new TextRun('כל אדם שיהיה זכאי על פי צוואה זו, ויתנגד לה או יערער עליה בכל דרך שהיא, יאבד את כלל זכויותיו לירושה על פי צוואה זו, ויקבל במקום זאת סכום סימלי של שקל אחד (₪1) בלבד.')
          ]
        })
      );
      ...
      sections.push(
        new Paragraph({
          children: [
            new TextRun(`נחתם בעיר: ${willData.willDate?.city || '[עיר]'}, היום ${willData.willDate?.day || '[תאריך]'} בחודש ${willData.willDate?.month || '[חודש]'}, ${willData.willDate?.year || '[שנה]'}.`)
          ],
      ...
      sections.push(
        new Paragraph({
          children: [
            new TextRun(`מעידים בזאת שהמצווה: ${willData.testator?.fullName || '[שם מלא]'}, חתם בפנינו מרצונו הטוב והחופשי, בהיותו בדעה צלולה ולאחר שיקול דעת, ולאחר שהצהיר בנוכחותנו כי זו צוואתו.`)
          ],
          spacing: { after: SPACING.betweenParagraphs }
        })
      );
    ...
    <div className={`${className}`}>
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-400 rounded-lg p-6 shadow-lg">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition disabled:opacity-50 disabled:cursor-not-allowed font-bold text-lg shadow-xl hover:shadow-2xl"
        >
          {isExporting ? (
            <>
            ...
            <>
              ...
              <span>יצוא צוואה מקצועית לWord - גרסה חדשה!</span>
            </>
          )}
        </button>
